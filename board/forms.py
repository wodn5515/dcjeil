...
class PostSuperuserForm(forms.ModelForm):
    ...
    date = forms.DateField(
        ...
        help_text="◈ 설교, 찬양, 기도에만 작성하세요.",
    )
    preacher = forms.CharField(
        label="설교자",
        required=False,
        widget=forms.TextInput(attrs={"class": "half", "placeholder": "ex)김대환 목사"}),
        help_text="◈ 설교에만 작성하세요.",
    )
    words = forms.CharField(
        label="말씀",
        required=False,
        widget=forms.TextInput(attrs={"class": "half", "placeholder": "ex)창세기 1:1~10"}),
        help_text="◈ 설교에만 작성하세요.",
    )
...
PostFileFormset = forms.modelformset_factory(
    PostFile, extra=0, max_num=5, fields=("file",), can_delete=True
)
